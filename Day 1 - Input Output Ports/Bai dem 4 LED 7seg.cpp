...
int main() {
    ...
    long nCntX = 0;          // 7seg LED1 counter
    ...
    while(1) {
        ...
        for(int delay=0;delay<20;delay++){
            for(int i=0;i<70;i++){
                EN_LED = 0b1000;
                LED7seg =  ~seg[nCntX/1000];          // Hàng nghìn
            }
            for(int i=0;i<70;i++){
                EN_LED = 0b0100;
                LED7seg =  ~seg[(nCntX%1000)/100];    // Hàng trăm
            }
            for(int i=0;i<70;i++){
                EN_LED = 0b0010;
                LED7seg =  ~seg[((nCntX%1000)%100)/10];    // Hàng chục
            }
            for(int i=0;i<70;i++){
                EN_LED = 0b0001;
                LED7seg =  ~seg[((nCntX%1000)%100)%10];    // Hàng đơn vị
            }
        }
                     
        nCntX++;
        // 7seg LED limit count 999
        if (1000 == nCntX) {
            nCntX = 0;
        }
